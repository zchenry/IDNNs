...
def calc_by_sampling_neurons(ws_iter_index, snaps, label, sigma, bins, pxs):
    iter_infomration = []
    for j in range(len(ws_iter_index)):
        data = ws_iter_index[j]
        new_data = np.zeros((snaps * data.shape[0], data.shape[1]))
        labels = np.zeros((snaps * label.shape[0], label.shape[1]))
        x = np.zeros((snaps * data.shape[0], 2))
        for i in range(data.shape[0]):
            cov_matrix = np.eye(data[i, :].shape[0]) * sigma
            t_i = np.random.multivariate_normal(data[i, :], cov_matrix, snaps)
            new_data[snaps * i:(snaps * (i + 1)), :] = t_i
            labels[snaps * i:(snaps * (i + 1)), :] = label[i, :]
            x[snaps * i:(snaps * (i + 1)), 0] = i
...
def calc_information_for_epoch(iter_index, ws_iter_index, bins, unique_inverse_x,
    ...
    if calc_full_and_vartional:
        ...
        params = [
            calc_varitional_information(ws_iter_index[i], label, model_path, i, len(ws_iter_index) - 1, iter_index,
                                        input_size, layers, ss[i], pys, ks[i], search_sigma=True) for i in
            range(len(ws_iter_index))]
    # Calc infomration of only subset of the neurons
    elif calc_information_by_sampling:
        parmas = calc_by_sampling_neurons(ws_iter_index=ws_iter_index, snaps=snaps, label=label,
