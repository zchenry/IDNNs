...
def plot_all_epochs(gen_data, I_XT_array, I_TY_array, axes, snapepochs, f,
                    index_i, index_j, size_ind, font_size, y_ticks, x_ticks,
                    colorbar_axis, title_str, axis_font, bar_font, save_name,
                    plot_error=True, index_to_emphasis=1000):
    if plot_error:
        ...
        for i in range(len(fig_data)):
            ax1.plot(snapepochs, fig_data[i],':',
                     linewidth = 3 , label = fig_strs[i])
        ax1.legend(loc='best')
    ...
    colors = [cmap(i) for i in np.linspace(0, 1, snapepochs[max_index-1]+1)]
    ...
    for index_in_range in range(0, max_index):
        ...
        if snapepochs[index_in_range] ==index_to_emphasis:
            axes[index_i, index_j].plot(XT, TY, marker='o', linestyle=None, markersize=19, markeredgewidth=0.04,
                ...
                axes[index_i, index_j].plot(XT[:], TY[:], marker='o', linestyle='-', markersize=12, markeredgewidth=0.01, linewidth=0.2,
                                color=colors[int(snapepochs[index_in_range])])
    ...
    if index_i ==axes.shape[0]-1 and index_j ==axes.shape[1]-1:
        utils.create_color_bar(f, cmap, colorbar_axis, bar_font, snapepochs, title='Epochs')
...
def plot_by_training_samples(I_XT_array, I_TY_array, axes, snapepochs, f,
    ...
    if index_i == axes.shape[0] - 1 and index_j == axes.shape[1] - 1:
        utils.create_color_bar(f, cmap, colorbar_axis, bar_font, snapepochs, title='Training Data')
...
def update_line_each_neuron(num, print_loss, Ix, axes, Iy, train_data, accuracy_test, epochs_bins, loss_train_data, loss_test_data, colors, snapepochs,
    ...
    title_str = 'Information Plane - Epoch number - ' + str(snapepochs[num])
    ...
    if len(axes)>1:
        axes[1].plot(snapepochs[:num], 1 - np.mean(accuracy_test[:, :num], axis=0), color='g')
        if print_loss:
            axes[1].plot(snapepochs[:num], np.mean(loss_test_data[:, :num], axis=0), color='y')
        nereast_val = np.searchsorted(epochs_bins, snapepochs[num], side='right')
        axes[1].set_xlim([0,epochs_bins[nereast_val]])
        axes[1].legend(('Accuracy', 'Loss Function'), loc='best')

def update_line(num, print_loss, data, axes, snapepochs, test_error, test_data, epochs_bins, loss_train_data, loss_test_data, colors,
                font_size = 18, axis_font=16, x_lim = [0,12.2], y_lim=[0, 1.08], x_ticks = [], y_ticks = []):
    """Update the figure of the infomration plane for the movie"""
    #Print the line between the points
    ...
    axes[1].plot(snapepochs[:num], 1 - np.mean(test_error[:, :num], axis=0), color ='r')
...
def load_figures(mode, str_names=None):
    if mode == 0:
        ...
        f.subplots_adjust(left=0.09, bottom=0.15, right=.928,
                          top=0.94, wspace=0.03, hspace=0.04)
        yticks = [0, 1, 2, 3]
        xticks = [3, 6, 9, 12, 15]
    elif mode == 1: # for 3 rows with 2 colmus
        font_size = 34
        axis_font = 26
        bar_font = 28
        fig_size = (14, 19)
        ...
        title_strs = [['One hidden layer', 'Two hidden layers'],
                      ['Three hidden layers', 'Four hidden layers'],
        ...
        f.subplots_adjust(left=0.09, bottom=0.08, right=.92,
                          top=0.94, wspace=0.03, hspace=0.15)
        colorbar_axis = [0.93, 0.1, 0.035, 0.76]
        sizes = [[1010, 1010], [1017, 1020], [1700, 920]]
    elif mode == 11: # for 2 rows with 3 colmus
        font_size = 34
        axis_font = 26
        bar_font = 28
        fig_size = (14, 9)
        xticks = [1, 3, 5, 7, 9, 11]
        yticks = [0, 0.2, 0.4, 0.6, 0.8, 1]
        title_strs = [['One hidden layer',
                       'Two hidden layers','Three hidden layers'],
                      ['Four hidden layers',
                      'Five hidden layers', 'Six hidden layers']]
        title_strs = [['5 Bins', '10 Bins', '15 Bins'],
                      ['20 Bins', '25 Bins', '35 Bins']]
        f, (axes) = plt.subplots(2, 3, sharex=True,
                                 sharey=True, figsize=fig_size)
        f.subplots_adjust(left=0.09, bottom=0.1, right=.92,
                          top=0.94, wspace=0.03, hspace=0.15)
        colorbar_axis = [0.93, 0.1, 0.035, 0.76]
        sizes = [[1010, 1010, 1017], [1020, 1700, 920]]
    if mode == 2 or mode == 6: # one figure
        axis_font = 28
        bar_font = 28
        fig_size = (14, 10)
        font_size = 34
        f, (axes) = plt.subplots(1, len(str_names),
                                 sharey=True, figsize=fig_size)
        if len(str_names) == 1:
            axes = np.vstack(np.array([axes]))
        f.subplots_adjust(left=0.097, bottom=0.12, right=.87,
                          top=0.99, wspace=0.03, hspace=0.03)
        colorbar_axis = [0.905, 0.12, 0.03, 0.82]
        xticks = [1, 3, 5, 7, 9, 11]
        yticks = [0, 0.2, 0.4, 0.6, 0.8, 1]
        sizes = [[-1]]
        title_strs = [['', '']]
    if mode == 3: # one figure with error bar
        fig_size = (14, 10)
        font_size = 36
        axis_font = 28
        bar_font = 25
        ...
        if len(str_names) == 1:
            axes = np.vstack(np.array([axes]))
        f.subplots_adjust(left=0.097, bottom=0.12, right=.90,
                          top=0.99, wspace=0.03, hspace=0.03)
        sizes = [[-1]]
        colorbar_axis = [0.933, 0.125, 0.03, 0.83]
        xticks = [0, 2, 4, 6, 8, 10, 12]
        yticks = [0.3, 0.4, 0.6, 0.8, 1]
    if mode == 4: # two figures second
        font_size = 27
        axis_font = 18
        bar_font = 23
        fig_size = (14, 6.5)
    ...
    return font_size, axis_font, bar_font, colorbar_axis, \
        sizes, yticks, xticks,title_strs, f, axes
...
def plot_norms(axes, snapepochs, norms1, norms2):
    ...
    axes.plot(snapepochs, np.mean(norms1[:, 0, 0, :], axis=0), color='g')
    axes.plot(snapepochs, np.mean(norms2[:, 0, 0, :], axis=0), color='b')
...
def plot_hist(str_name, save_name='dist'):
    ...
    ind_array = data_array['params']['snapepochs']
...
if __name__ == '__main__':
    ...
    prex = 'jobsFiles/'
    sofix = '.pickle'
    prex2 = '/Users/henry/Documents/idnn/jobs/'
    ...
    if action == TIME_STEMPS or action == MOVIE:
        index = 1
        name_s = prex2+ 'g_layers=10,7,5,4,3_lastepoch=9998_nRepeats=40_batchsize=3563_DataName=var_u_nEpoch=10000_lr=0.0002_nEpochInds=964_samples=1_nDistSmpls=1/'
        name_s = prex2 +'r_DataName=MNIST_sampleLen=1_layers=400,200,150,60,50,40,30_lr=0.0002_nEpochInds=677_nRepeats=1_LastEpochsInds=1399_nDistSmpls=1_nEpoch=1400_batch=2544/'
        ...
        if action == ALL_LAYERS:
            ...
            str_names[1][2] = prex2+'g_layers=10,7,5,4,4,3_LastEpochsInds=9998_nRepeats=20_batch=3563_DataName=var_u_nEpoch=10000_lr=0.0004_nEpochInds=964_samples=1_nDistSmpls=1/'
            str_names = [[prex2 +'nbins8_DataName=var_u_sampleLen=1_layers=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/',
                          prex2 +'nbins12_DataName=var_u_sampleLen=1_layers=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/',
                          prex2 +'nbins18_DataName=var_u_sampleLen=1_layers=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/'],
                         [prex2 +'nbins25_DataName=var_u_sampleLen=1_layers=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistrSmpls=1_nEpoch=10000_batch=4096/',
                           prex2 +'nbins35_DataName=var_u_sampleLen=1_layers=10,7,5, 4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/',
                           prex2 + 'nbins50_DataName=var_u_sampleLen=1_layers=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/']]
