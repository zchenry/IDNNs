...
def multi_layer_perceptron(x, n_inputs, n_classes, n_hidden_1, n_hidden_2):
    hidden = []
    inputs = []
    hidden.append(x)
    # Network Parameters
    # n_inputs = x.shape[0]  # MNIST data inputs (img shape: 28*28)
    ...
    layer_1_inputs = tf.add(tf.matmul(x, weights['h1']), biases['b1'])
    layer_1 = tf.nn.relu(layer_1_inputs)
    inputs.append(layer_1)
    hidden.append(layer_1)

    layer_2_inputs = tf.add(tf.matmul(layer_1, weights['h2']), biases['b2'])
    layer_2 = tf.nn.relu(layer_2_inputs)
    inputs.append(layer_2_inputs)
    hidden.append(layer_2)

    inputs_y = tf.matmul(layer_2, weights['out']) + biases['out']
    y_output = tf.nn.softmax(inputs_y)
    inputs.append(y_output)
    hidden.append(y_output)
    return y_output, hidden, inputs
...
def deepnn(x):
    ...
    with tf.name_scope('conv1'):
        ...
        with tf.name_scope('act'):
            ...
            tf.summary.histogram('act', h_conv1)
    ...
    with tf.name_scope('conv2'):
        ...
        with tf.name_scope('act'):
            ...
            tf.summary.histogram('act', h_conv2)
        with tf.name_scope('max_pol'):
    ...
    with tf.name_scope('FC1'):
        ...
        with tf.name_scope('act'):
            ...
            tf.summary.histogram('act', h_fc1)
